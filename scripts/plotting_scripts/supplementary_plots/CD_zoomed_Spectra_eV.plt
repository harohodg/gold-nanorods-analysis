...
std=1
root_folder='..'
file_name=sprintf('CD_zoomed_Spectra_ev_multi_std.png')
...
std=0.2
function(i, file, std)=sprintf("<  if [ -f '%s' ];then awk  'function abs(v) {return v < 0 ? -v : v} BEGIN{ORS=\" \";print \"f%d(x)=0\"}NR>1&&$1>=1.70&&$1<=4.10{print \" + Sde(\"  $7  \",\" $1 \", %.2f ) * gaussian(x, \" $1 \", %.2f) \" }' %s;else echo 'f%d(x)=0';fi", file, i, std, std, file, i)
...
set yrange [SCALE_MIN*1.05:SCALE_MAX*1.05]
