...
std=1
root_folder='..'
file_name=sprintf('AB_zoomed_Spectra_ev_multi_std.png')
...
std=0.2
function(i, file, std)=sprintf("<  if [ -f '%s' ];then awk  'function abs(v) {return v < 0 ? -v : v} BEGIN{ORS=\" \";print \"f%d(x)=0\"}NR>1&&$1>=1.70&&$1<=4.10{print \" + Se(\"  $3  \", %.2f)  * gaussian(x, \" $1 \", %.2f) \" }' %s;else echo 'f%d(x)=0';fi", file, i, std, std, file, i)


load function(1, data_file1, 0.2)
load function(2, data_file2, 0.2)
load function(3, data_file3, 0.2)
load function(4, data_file4, 0.2)
load function(5, data_file5, 0.2)
