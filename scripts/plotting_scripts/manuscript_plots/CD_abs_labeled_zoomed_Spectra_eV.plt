...
std=1.0
root_folder='..'
file_name=sprintf('CD_abs_labeled_zoomed_Spectra_eV_std=%.2f.png',std)
xmax=6
num_samples=1000
vpixels=1080
hpixels=1920
...
function(i, file)=sprintf("<  if [ -f '%s' ];then awk  'function abs(v) {return v < 0 ? -v : v} BEGIN{ORS=\" \";print \"f%d(x)=0\"}NR>1&&$1>=1.70&&$1<=4.10{print \" + Sde(\"  abs($7)  \",\" $1 \", std ) * gaussian(x, \" $1 \", std) \" }' %s;else echo 'f%d(x)=0';fi", file, i, file, i)
...
set key below
set title "Smoothed AVCD Spectra" offset 0,title_offset
set xlabel "eV"
...
#smoothed_plot_1_title=sprintf("    %s - %.2f eV", plot_1_title, STATS_pos_max_y)
print(plot_1_title, STATS_max_y)

set table 'stats.dat'
plot f2(x)
unset table
stats 'stats.dat' nooutput
#smoothed_plot_2_title=sprintf("    %s - %.2f eV", plot_2_title, STATS_pos_max_y)
print(plot_2_title, STATS_max_y)

set table 'stats.dat'
plot f3(x)
unset table
stats 'stats.dat' nooutput
#smoothed_plot_3_title=sprintf("    %s - %.2f eV", plot_3_title, STATS_pos_max_y)
print(plot_3_title, STATS_max_y)

set table 'stats.dat'
plot f4(x)
unset table
stats 'stats.dat' nooutput
#smoothed_plot_4_title=sprintf("    %s - %.2f eV", plot_4_title, STATS_pos_max_y)
print(plot_4_title, STATS_max_y)

set table 'stats.dat'
plot f5(x)
unset table
stats 'stats.dat' nooutput
#smoothed_plot_5_title=sprintf("    %s - %.2f eV", plot_5_title, STATS_pos_max_y)
print(plot_5_title, STATS_max_y)
...
unset xlabel
set bmargin 0
set yrange [0:MAX*1.5]


set bmargin at screen get_bmargin(2)
set tmargin at screen get_tmargin(2)
set title plot_1_title offset 0,title_offset
plot data_file1 using  (column(xcolumn)):((column(xcolumn) >= low && column(xcolumn) <= high ? abs(column(ycolumn)) : 1/0)) with impulses

set bmargin at screen get_bmargin(3)
set tmargin at screen get_tmargin(3)
set title plot_2_title offset 0,title_offset
plot data_file2 using  (column(xcolumn)):((column(xcolumn) >= low && column(xcolumn) <= high ? abs(column(ycolumn)) : 1/0)) with impulses

set bmargin at screen get_bmargin(4)
set tmargin at screen get_tmargin(4)
set title plot_3_title offset 0,title_offset
plot data_file3 using  (column(xcolumn)):((column(xcolumn) >= low && column(xcolumn) <= high ? abs(column(ycolumn)) : 1/0)) with impulses

set bmargin at screen get_bmargin(5)
set tmargin at screen get_tmargin(5)
set title plot_4_title offset 0,title_offset
plot data_file4 using  (column(xcolumn)):((column(xcolumn) >= low && column(xcolumn) <= high ? abs(column(ycolumn)) : 1/0)) with impulses

set bmargin at screen get_bmargin(6)
set tmargin at screen get_tmargin(6)
set title plot_5_title offset 0,title_offset
plot data_file5 using  (column(xcolumn)):((column(xcolumn) >= low && column(xcolumn) <= high ? abs(column(ycolumn)) : 1/0)) with impulses
