...
std=1
root_folder='..'
file_name=sprintf('AB_raw_labeled_zoomed_Spectra_eV_std=%.2f.png',std)
...
function(i, file)=sprintf("<  if [ -f '%s' ];then awk  'function abs(v) {return v < 0 ? -v : v} BEGIN{ORS=\" \";print \"f%d(x)=0\"}NR>1&&$1>=1.70&&$1<=4.10{print \" + Se(\"  $3  \", std)  * gaussian(x, \" $1 \", std) \" }' %s;else echo 'f%d(x)=0';fi", file, i, file, i)
...
set xlabel "eV"
set ylabel "ε(L mol^{-1} cm^{-1})" offset -4,0
